...
async function sendCSV(id) {
  ...
  if (registros.length === 0) {
    await send(id, "No hay registros aun!");
    return;
  }
...
function detectarFecha(msg) {
  ...
  if (low.indexOf("ayer") >= 0) {
    fecha.setDate(fecha.getDate() - 1);
  } else if (low.indexOf("lunes") >= 0) {
    fecha.setDate(fecha.getDate() - ((fecha.getDay() + 6) % 7));
  } else if (low.indexOf("martes") >= 0) {
    fecha.setDate(fecha.getDate() - ((fecha.getDay() + 5) % 7));
  } else if (low.indexOf("miercoles") >= 0 || low.indexOf("miercoles") >= 0) {
    fecha.setDate(fecha.getDate() - ((fecha.getDay() + 4) % 7));
  } else if (low.indexOf("jueves") >= 0) {
    fecha.setDate(fecha.getDate() - ((fecha.getDay() + 3) % 7));
  } else if (low.indexOf("viernes") >= 0) {
    fecha.setDate(fecha.getDate() - ((fecha.getDay() + 2) % 7));
  } else if (low.indexOf("sabado") >= 0) {
    fecha.setDate(fecha.getDate() - ((fecha.getDay() + 1) % 7));
  } else if (low.indexOf("domingo") >= 0) {
    fecha.setDate(fecha.getDate() - (fecha.getDay() % 7));
  }
...
function programarReporte() {
  ...
  setTimeout(function() { reporteMensual().then(function() { programarReporte(); }); }, diff);
}
...
app.post("/webhook", async function(req, res) {
  ...
  try {
    ...
    const text = m.text;
    if (text === "/start") {
      await send(id, "Hola! Soy ContaBot tu contador. Dime tus ventas, gastos o compras. Usa /excel para descargar tu reporte!");
      return;
    }
    if (text === "/reset") {
      hist[id] = [];
      datos[id] = [];
      await send(id, "Historial limpiado!");
      return;
    }
    if (text === "/excel") {
      await send(id, "Generando tu archivo...");
      await sendCSV(id);
      return;
    }
